...
class Lien(discord.ui.View):
    ...
    def __init__(self, label: str, url: str):
        super().__init__()
        self.add_item(discord.ui.Button(label=label, url=url, emoji="<:unknown:1223283454901489756>"))


class SelectMenu(discord.ui.Select):
    def __init__(self, restaurant: dict, menus: list, theme: str, options: list, map: str, link: tuple[str, str]):
        ...
        if len(options) > 0:
            super().__init__(placeholder="Choisissez un menu", options=options, min_values=1, max_values=1, row=0)
        else:
            super().__init__(placeholder="Aucun menu disponible", options=[discord.SelectOption(label="Aucun menu disponible", value="null", emoji="<:unknown:1223283454901489756>", default=True)], min_values=1, max_values=1, row=0)
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            return await interaction.response.send_message(content="Erreur lors de la récupération du menu", ephemeral=True)
...
class MenuView(discord.ui.View):
    ...
    Bouton de menu
    """
    def __init__(self, restaurant: dict, menu: dict, menus: list, options: list, map: str, link: tuple[str, str], interaction: discord.Interaction = None, ephemeral: bool = False):
    ...
    @discord.ui.button(emoji="<:list:1223283450904317962>", label="Menu", style=discord.ButtonStyle.gray, row=1)
    async def menuButton(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            if not self.menu:
                embed = discord.Embed(
                ...
                menu = await interaction.client.entities.menus.getFromDate(
                    id=self.restaurant.get("rid"),
                    date=self.menu.get("date")
                )
                ...
                for meal in menu_per_day[self.menu.get("date").strftime("%d-%m-%Y")].get("repas"):
                    count = 0
                    for category in meal.get("categories"):
                        ...
                        embed.add_field(name=category.get("libelle"), value=text, inline=False)
                        count += 1

                        if len(meal.get("categories")) > 25 and count == 24:
                            embed.add_field(name="\u2060", value=f"Et {len(meal.get('categories')) - count} autres categories", inline=False)
                            break
            ...
            interaction.client.logger.error(f"Error during menu display: {self.restaurant.get('rid')} - {e}")
            interaction.client.logger.error(traceback.format_exc())
