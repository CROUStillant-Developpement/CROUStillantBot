...
async def region_autocomplete(
    ...
    return [
        ...
        for region in interaction.client.cache.regions if current.lower() in region.get('libelle').lower() or current.lower() in str(region.get('idreg'))
    ][:25]
...
async def restaurant_autocomplete(
    ...
    return [
        ...
        for restaurant in interaction.client.cache.restaurants if current.lower() in restaurant.get('nom').lower() or current.lower() in str(restaurant.get('rid'))
    ][:25]
